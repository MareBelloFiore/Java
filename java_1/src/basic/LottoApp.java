...
//1~45 범위의 정수형 난수를 6개 제공받아 출력하는 프로그램
// => 6개의 난수값은 서로 중복되지 않도록 작성하며 오름차순 정렬하여 출력
// 정렬(sort) : 값을 차례대로 나열하는 기능 - 오름차순 / 내림차순
public class LottoApp {
	public static void main(String[] args) {
		//6개의 정수 난수값을 저장하기 위한 배열 생성
		int[] lotto=new int[6];
		
		//1~45 범위의 정수 난수를 6개 제공받아 배열 요소에 차례로 저장
		for(int i=0; i<lotto.length; i++) { // 새로운 난수를 저장할 배열 요소의 표현한 인덱스의 반복
			...
			while(true) {
				...
				// 새로운 난수값과 기존 난수값이 중복되지 않는 경우 while문 종료
				if(!result) break;	
			}
		}
		...
		for(int i=0;i<lotto.length;i++) {// 비교하기 위한 배열 요소의 첨자를 표현하는 반복문 : 0 ~ 끝-1
